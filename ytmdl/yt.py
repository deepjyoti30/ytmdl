...
def _is_verified(desc: List) -> bool:
    ...
    This can be determined in a few ways:
        - If the description contains `Auto-Generated by YouTube`
        - If the description contains `Provided to YouTube by <company>`
    """
    if desc is None:
        return False
...
def scan_video(url, proxy):
    """Scan the link of the video and return data and proxies."""
    proxies = {}
    if proxy is not None:
        proxies['http'] = proxy
    try:
...
def is_playlist(url):
    """
    Check if the passed URL is a Youtube playlist URL.
    """
...
def get_title(url, ytdl_config: str = None) -> str:
    ...
    # Boolean denoting if the user needs to verify the song 
    # title (initially assume title is automatically handled)
    verify_title = False
    
    # Try to get the title by using Youtube Music
    try:
